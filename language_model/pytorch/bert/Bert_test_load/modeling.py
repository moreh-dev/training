...
try:
    ...
    bias_gelu = moreh_ops.gelu
    print("Use Moreh's gelu")
except:
    from layers.fused_gelu_orig import bias_gelu_impl as bias_gelu
    print("Use (BERT) Pytorch gelu")
    
#from layers.fused_gelu_orig import bias_gelu_impl as bias_gelu
#from utils import get_rank
from utils_bert import get_rank #TUAN

logger = logging.getLogger(__name__)

moreh_save_prefix = "moreh-6gpu-6-batch-fine-grained-save-for-reporting-bias"
...
def load_tf_weights_in_bert(model, tf_checkpoint_path, use_fast_mha=False):
    ...
    """
    assert use_fase_mha == False and "(jinpyo) MHA removed"
    try:
        import re
        import numpy as np
...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, force_save=False):
        ...
        moreh_save(self.LayerNorm.bias, 'ln-bias.pt', force_save=force_save)
...
class FastBertAttention(nn.Module):
    def __init__(self, config):
        ...
        self.multi_head_attention = nn.MultiheadAttention(config.hidden_size, config.num_attention_heads, dropout = config.attention_probs_dropout_prob, bias=True)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(self, input_tensor, attention_mask):
        ...
        multi_head_attention_output,_ = self.multi_head_attention(query = input_tensor, key = input_tensor, value = input_tensor, key_padding_mask=attention_mask, need_weights=True,attn_mask = None)
...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states, force_save=False):
...
class BertPreTrainingHeads(nn.Module):
    ...
    def forward(self, sequence_output, pooled_output, masked_lm_labels):
        #print ("sequence_output1: ", sequence_output.shape) #[8, 512, 1024]
        if self.dense_seq_output:
            # We are masking out elements that won't contribute to loss because of masked lm labels
            #sequence_flattened = torch.index_select(sequence_output.view(-1,sequence_output.shape[-1]), 0, torch.nonzero(masked_lm_labels.view(-1) != -1, as_tuple=False).squeeze()) #orig
            #temp = torch.index_select(sequence_output.view(-1,sequence_output.shape[-1]), 0, torch.nonzero(masked_lm_labels.view(-1), as_tuple=False).squeeze()) #working 
            #print ("masked_lm_label: ", masked_lm_labels.shape)

            temp = sequence_output.view(-1,sequence_output.shape[-1]) #checking if nonzero can be dropped
            #print ("temp1: ", temp1.shape) #
            #print ("equal: ", torch.equal(temp, temp1))

       
            #sequence_output = sequence_flattened #orig
            sequence_output = temp 
        #print ("sequence_output2: ", sequence_output.shape) #[297, 1024]
        prediction_scores = self.predictions(sequence_output)
        seq_relationship_score = self.seq_relationship(pooled_output)
...
class BertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_checkpoint, state_dict=None, cache_dir=None,
        ...
        if from_tf:
            ...
            return load_tf_weights_in_bert(model, pretrained_checkpoint, use_fast_mha=False)
...
class BertForPreTraining(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
            ...
            checkpoint_activations=False, force_save=False):
        ### kh add ###
        ...
        if self.dense_seq_output:
            ...
            #print ("masked_lm_labels_flat: ", masked_lm_labels_flat.shape)
            #mlm_labels = masked_lm_labels_flat[masked_lm_labels_flat != -1] #orig
            #mlm_labels = masked_lm_labels_flat[masked_lm_labels_flat]
            mlm_labels = masked_lm_labels_flat #working
        ...
        if masked_lm_labels is not None and next_sentence_label is not None:
            ...
            if self.dense_seq_output:
                ...
                #print ("prediction_scores: ", prediction_scores.view(-1, self.config.vocab_size).shape)
                #print ("mlm_labels: ", mlm_labels.view(-1).shape)
            ...
            if self.training:
            #if True:
                #print ("mlm_predictions: ", mlm_predictions.shape)
                #print ("mlm_labels: ", mlm_labels.shape)
                mlm_acc = (mlm_predictions == mlm_labels).sum(dtype=torch.float)/mlm_labels.numel() # TUAN: CHEKC ALL mlm_labels.sum...
            else:
                #print ("masked_lm_labels_flat: ", masked_lm_labels_flat)
                #temp = torch.where(masked_lm_labels_flat != -1, torch.full_like(masked_lm_labels_flat, 1), 0) # FIXED 1


                #temp = torch.where(masked_lm_labels_flat != -1, torch.full_like(masked_lm_labels_flat, 1), torch.full_like(masked_lm_labels_flat, 0)) # 
                #mlm_acc = (mlm_predictions == mlm_labels).sum(dtype=torch.float)/temp.sum(dtype=torch.float) # NaN values raises after few iterations 

                #total = mlm_labels.numel()
                #print ("isnan(mlm_labels full): ", torch.sum(mlm_labels.isnan()))
                #print ("isnan(mlm_predictions full): ", torch.sum(mlm_predictions.isnan()))
                _sum = torch.sum(mlm_labels != -1)
                mlm_acc = (mlm_predictions == mlm_labels).sum(dtype=torch.float)/_sum # TUAN is trying to fix with sum only
                #print ("isnan(mlm_labels getitem): ", mlm_acc)
            ...
            if self.training:
                ...
                #temp = torch.where(mlm_labels != -1, torch.full_like(masked_lm_labels_flat, 1), torch.full_like(masked_lm_labels_flat, 0)) #NaN after few iterations
                #return total_loss, mlm_acc, temp.sum(dtype=torch.float) 
                ...
                #return total_loss, mlm_acc, mlm_labels[mlm_labels != -1].numel()
